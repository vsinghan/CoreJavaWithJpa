			...
			case 2:
				...
				for (;;) {
					for (int i = 0; i < noOfPassengers; i++) {
						System.out.println("\nFor passenger " + (i + 1) + " Choose any seat no. between 501 - 680:\n");
						try {
							seatNo = Integer.parseInt(scanner.nextLine());
						} catch (NumberFormatException e) {
							count++;
							if (count == 3) {
								System.out.println("\nYour attempts limit is exhausted!!");
					...
					try {
						List<Integer> tickets = bookingOperations.bookTicket(passengerList, seatNumbers);
			...
			case 3:
				...
				for (;;) {
					...
					try {
						...
						System.out.println("\nTicket Id should be a number between 956 - 1155!!!");
						count++;
						if (count == 3) {
							System.out.println("\nYour attempts limit is exhausted!!");
							break;
