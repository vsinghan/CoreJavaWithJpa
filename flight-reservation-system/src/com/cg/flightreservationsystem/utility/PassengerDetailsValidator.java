...
public class PassengerDetailsValidator {
	...
	public void isSeatNoValid(int seatNo) throws FRSException {
		int nameCount = 0;
		//
		// Pattern pattern = Pattern.compile("^[5-6]{1}[1-8]{1}[0-9]{1}$");
		//
		// Matcher m = pattern.matcher(seatNo + "");
		//
		// if (!m.find()) {
		if (seatNo < 501 && seatNo > 680)
			throw new FRSException("Passenger " + ++nameCount + " Seat no. is Invalid");
		// }
	}
	...
	public boolean isTicketIdValid(String ticketId) {
		// Pattern pattern = Pattern.compile("^[1]{1}[0-1]{1}[1-8]{1}[0-9]{1}$");
		// Matcher m = pattern.matcher(ticketId);

		// if (m.find())
		if (Integer.parseInt(ticketId) > 955 && Integer.parseInt(ticketId) < 955 + 180) {
			return true;
		} else {
			return false;
		}
